using codingTracker._0lcm.Extensions;
...
internal static class DisplayHelper
{
    ...
    internal const string Red = "#cd2d2d";
    internal const string Yellow = "#e2b929";
    internal const string Error = "#870c00";
    ...
    internal static T DisplayMenu<T>(string? title = null) where T : Enum
    {
        var menuChoice = AnsiConsole.Prompt(
            new SelectionPrompt<T>()
                ...
                .UseConverter(e => EnumExtensions.ToDisplayString(e))
                );
    ...
    internal static List<string> DisplayMultiPrompt(List<string> choiceList, string? title = null, bool requireChoice = true)
    {
    ...
    internal static void DisplaySpinner(string waitMessage, int waitTimeInMs = 3000)
    {
        AnsiConsole.Status()
            .Spinner(Spinner.Known.Star)
            .Start($"[{White}]{waitMessage}[/]", ctx =>
            {
                Thread.Sleep(waitTimeInMs);
            });
    }
    internal static async Task DisplayAsyncSpinner(string waitMessage, Task task)
    {
        await AnsiConsole.Status()
            .Spinner(Spinner.Known.Star)
            .StartAsync($"[{White}]{waitMessage}[/]", async ctx =>
            {
                await task;
            });
    }
